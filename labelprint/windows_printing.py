...
def print_content(content, printer, method='print-cmd', job_description=None, verbose=0):
    ...
    if printer[:2] != r"\\":
        print("\nWARNING: The given printer, '{}', does not use a fully-qualified name. ".format(printer),
              "This may not work. \n",
    ...
    if " " in printer:
        print("\nWARNING: The given printer, '{}', includes spaces. This may not work! ".format(printer),
    ...
    if method == 'win32print':
        if verbose:
            print(("Printing {} {} "
                   "to printer '{}' using {!r}...").format(
                len(content), 'characters' if isinstance(content, str) else 'bytes', printer, method),
                  file=sys.stderr)
    ...
    if verbose:
        print(("Printing {} {} "
               "in file '{}' to printer '{}' using {!r}...").format(
            len(content), 'chars' if isinstance(content, str) else 'bytes',
            file.name, printer, method
        ), file=sys.stderr)
    if method == 'print-cmd':
...
def execute_and_process(cmd_args, capture_output=True, check=True, verbose=1):
    ...
    # subprocess.run() was added in Python 3.5 - Sublime Text uses Python 3.3.
    output = subprocess.check_output(cmd_args, stderr=subprocess.STDOUT, shell=True)
    if output:
        print("\noutput from running %s:" % (cmd_args,))
        print(output)
    return output
...
def print_file_using_copy_cmd(
    ...
    Args:
        ...
        binary_mode: If True, copy the file in binary mode, e.g. `copy /b`.
        check: If True, check the returncode and raise error if non-zero.
        capture_output: Capture the output of the print executed command.
...
def print_file_using_print_cmd(
    ...
    """
    cmd_args = ["print", "/d:"+str(printer_name), filename]
    return execute_and_process(cmd_args, check=check, capture_output=capture_output)
...
def print_file_using_notepad_cmd(filename, printer_name, job_description=None, check=True, capture_output=True):
    ...
    Printing with notepad.exe:
        > notepad.exe /P <filename>
        > notepad.exe /PT <filename> <printername> <driverdll> <port> print filename to designated printer

        There is no switch to disable file menu functionality.

        Ref: https://answers.microsoft.com/en-us/windows/forum/windows_7-files/notepadexe-command-line-options/810760c1-a45a-4013-9544-1c1208e1b389

    """
    cmd_args = ["notepad", "/PT", filename, printer_name]
    return execute_and_process(cmd_args, check=check, capture_output=capture_output)
