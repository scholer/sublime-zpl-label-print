...
def print_zpl_labels(
    ...
    if sep == "\\t" or sep == "TAB":
        print("Converting escaped sep={!r} to regular tab character '\t'.".format(sep), file=sys.stderr)
        sep = "\t"

    # 1. Read input data
    if verbose:
        print("Reading input data from {} using sep={!r}...".format(source_type, sep), file=sys.stderr)
    if source_type == 'csvfile':
        if not data_file:
            raise ValueError("`data_file` ({}) must be specified when `source_type` is 'csvfile'.".format(data_file))
        data = data_from_csv_file(data_file, fieldnames=fieldnames, sep=sep)
    elif source_type == 'csvcontent':
        # `data_args` is text content readable by data_from_csv_content()
        data = data_from_csv_content(
            data_args, fieldnames=fieldnames, sep=sep, eol_char=eol_char)
        ...
        raise ValueError("Unrecognized value {!r} for argument `source_type`.".format(source_type))
    ...
    if formulas:
        if isinstance(formulas, str):
            formulas = get_formulas(formulas, sep=sep)
        # Else, assume that formulas is already a dict/OrderedDict of {field: callable} items.
    else:
        # If formulas is None, make it an empty dict for compatibility.
        formulas = {}
    if formulas:
    ...
    if save_to_file:
        if save_to_file == "-":
            ...
            with open(save_to_file, 'wb') as f:
                print("Writing zpl content to file", save_to_file, "...", file=sys.stderr)
                f.write(zpl_content)
